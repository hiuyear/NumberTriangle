...
public class NumberTriangle {
    ...
    public int retrieve(String path) {
        // td implement this method
        if (path.isEmpty()) {
            return this.root;
        } else {
            NumberTriangle curr = this;  // SUPER HARD! RMB THIS PART!!! SET CURR AS CURR STRUCTURE!!!
            char[] chars = path.toCharArray();
            for (char c : chars) {
                if (c == 'l' && curr.left != null) {
                    curr = curr.left;
                } else if (c == 'r' && curr.right != null) {  // NOT ELIF ANYMORE!!!
                    curr = curr.right;
                }
